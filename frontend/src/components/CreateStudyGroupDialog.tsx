...
export function CreateStudyGroupDialog({ onCreated }: CreateStudyGroupDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(_: unknown, data: { open: boolean }) => setOpen(data.open)}>
      <DialogTrigger disableButtonEnhancement>
        <Button appearance="primary" icon={<Add24Regular />}>Create Group</Button>
      </DialogTrigger>
      <DialogSurface>
        <DialogBody>
          ...
          <DialogActions>
            ...
            <Button
              ...
              onClick={() =>
                mutation.mutate({ name: name.trim(), description: description.trim(), subject, isPublic })
              }
            >
