...
export function BadgesWidget({ userId }: BadgesWidgetProps) {
    ...
    if (isLoading) return <div role="status" aria-live="polite" aria-label={t("badges.loading")}><Spinner size="tiny" label={t("badges.loading")} aria-hidden="true" /></div>;
    if (isError) {
        return (
    ...
    if (badges.length === 0) {
        return (
            <Body1 style={{ color: tokens.colorNeutralForeground3 }}>{t("badges.noBadges")}</Body1>
        );
    }
                ...
                {badges.map((badge) => (
                    <HoverLift key={badge.id}>
                        ...
                        >
                            ...
                            <Badge
                                appearance={badge.earned ? "filled" : "outline"}
                                color={badge.earned ? "brand" : "subtle"}
                            >
