...
function renderMarkdown(md: string): string {
  ...
  html = html.replace(/```([\s\S]*?)```/g, (_m, code: string) => `<pre><code>${code}</code></pre>`)
  // Images: ![alt](url)
 ...
 * Spec refs: F1.1–F1.7.
 */
export function MarkdownEditor({ value, onChange, placeholder = 'Write in Markdown...' }: MarkdownEditorProps) {
    ...
    <div className={styles.root}>
      ...
      {mode === 'edit' ? (
        ...
        <div>
          ...
          />
          <Caption1>
            Preview is a lightweight render; the server sanitizes stored content
            via OWASP HtmlSanitizer.
          </Caption1>
        </div>
      )}
    </div>
