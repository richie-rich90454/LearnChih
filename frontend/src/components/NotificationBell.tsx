  ...
  return (
    <Popover open={open} onOpenChange={(_, data) => setOpen(data.open)}>
      <PopoverTrigger disableButtonEnhancement>
        <Button appearance="subtle" icon={<Alert24Regular />} aria-label={`Notifications ${unreadCount > 0 ? `(${unreadCount} unread)` : ''}`}>
          {unreadCount > 0 && (
      ...
      <PopoverSurface className={styles.popoverSurface}>
          ...
          {notifications.map((n) => (
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', width: '100%', textAlign: 'left' }}>
                ...
                <Caption1 style={{ color: 'var(--colorNeutralForeground3)' }}>{n.message}</Caption1>
        ...
        {notifications.length > 0 && (
          <div className={styles.footer}>
            <Button appearance="transparent" size="small" onClick={() => { setOpen(false); navigate('/notifications') }}>
              View all
            </Button>
          </div>
        )}
